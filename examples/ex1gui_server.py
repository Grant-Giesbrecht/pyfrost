...
class PyfrostServerGUI(QMainWindow):
	...
	def __init__(self, log, app, *args, **kwargs):
		...
		self.label = QLabel("Uninitialized")
...
def server_stat_thread_main_gui(window):
	...
	while not server_opt.kill_stat_thread:
		...
		if time.time() - time_last_print > SERVER_STAT_PRINT:
			...
			window.label.setText(f"Active threads: {threading.active_count()}")
...
def server_main_gui(sock:socket, query_func:Callable[..., None]=None, send_func:Callable[..., None]=None, sa_init_func:Callable[..., None]=None):
...
if __name__ == "__main__":
	...
	server_thread = threading.Thread(target=server_main_gui, args=(sock, custom_func))
